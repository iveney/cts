...
// 		sleep(3);
		stepv = (stepv - (*(map+ibp->next))->delta_length_buf ) ;
		lastnode = ibp->next;
...
DME_TREE_NODE * stream_down(DME_TREE_NODE * tg, DME_TREE_NODE * src ,int * num ,int flag ){
DME_TREE_NODE * temp ; 
int loop  = 0; 
	if ( src == NULL){
...
DME_TREE_NODE * stream_up(DME_TREE_NODE * tg, DME_TREE_NODE * src , int  num, int flag ){
DME_TREE_NODE * temp ; 
DIRECTION  moveon; 
	if ( src == NULL){
...
DME_TREE_NODE * stream_down_stop(DME_TREE_NODE * tg, DME_TREE_NODE * src, int length ,int * num, int flag){
DME_TREE_NODE * temp ; 
DIRECTION moveon; 
int time = 0 ;
int  tour = length ;
...
//			printf(" altitude 1 is %d\n", altitude1);
//			printf(" altitude 2 is %d\n", altitude2);
...
//			printf(" round is %d\n",tour/(2*delta));
			if ( (round = (tour/(2*delta))) > 0){
				for ( i = 1; i <= round ; i++){
						round_num = 0 ;
						di = stream_down(di,ofa,&round_num,lrson);					 
						di = stream_up(di ,Troot,round_num,lrson);
				}
...
//			printf(" tour is %d\n",tour);
			if ( tour > 0){
				round_num = 0 ; 
				di = stream_down_stop(di,ofa,tour,&round_num,lrson);	
				if (round > 0)
					di = stream_up(di,di ,round_num,0);
				else 
					di = stream_up(di,di ,round_num,lrson);
			}
