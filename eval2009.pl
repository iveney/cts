...
# 2009/01/01   Created
#              - Thanks for the help (tcl examples) from Zhuo Li, lizhuo@us.ibm.com
# 
# units:
#   voltage  V
#   temp     deg celsius
...
for $i (0 .. ($numNode - 1)) {
  $_ = <RFILE>;
  die "ERROR" unless(/^\s*(\S+)\s+(\d+)\s+(\d+)\s*$/);
...
for $i (0 .. ($#nodeIdArr )) {
    if ($nodeSsIdArr[$i] ne $ssIdArr[0] && $nodeSsIdArr[$i] ne "__internal__" ) { 
        ...
        push @monitorTheseNodes, "$nodeIdArr[$j]" if $firstTime;
        $numSinkInMonitor++ if $firstTime;
    }
}
...
for ($k=3; $k<$numDataSet; $k++) {
    ...
    if ( $opt_s ){
        print "ERROR slew violation $slew[$k] at $monitorTheseNodes[$monitorTheseNodesCount]\n" if $slew[$k] > $SLEWLIMIT;
    } else {
        die "ERROR slew violation $slew[$k] at $monitorTheseNodes[$monitorTheseNodesCount]," if $slew[$k] > $SLEWLIMIT;
    }
    if ( $k >= $numDataSet - $numSinkInMonitor ) {
        printf ("    sink %s (%s) latency %.3f slew %.3f\n", $monitorTheseNodes[$monitorTheseNodesCount],$nodeSsIdArr[$CountFromNodeId{$monitorTheseNodes[$monitorTheseNodesCount]}],$d50[$k]-$d50[2],$slew[$k]) if ( $opt_v > 0 );
